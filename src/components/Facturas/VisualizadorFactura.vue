  ...
  <q-card flat>
    <q-card-section>
      <div :style="{'padding-top': stickyHeight + 'px'}" class="row items-center">
        ...
        <q-space/>
        <span class="comprobante">ID Factura: {{ factura.id }}</span>
      </div>
      <br>
      <responsive-table :data="factura.detalles" :schema="esquema" title="Items"/>
      <br>
      <div class="col-auto row items-end justify-end relative-position">
        ...
        <span
          class="col-auto q-pa-sm item-bordered shadow-2 total"><b>Total factura: ${{
            presentCurrency(factura.total)
          }}</b></span>
      </div>
      <br>
      <presentador>
        <q-input v-model="factura.fecha" label="Fecha" readonly/>
        <q-input v-model="factura.fechaVencimiento" label="Fecha Vencimiento" readonly/>
      </presentador>
      <br>
      <presentador>
        <q-input label="Nit/CC" v-model="factura.clienteID" readonly/>
        <q-input label="Nombre" v-model="factura.clienteNombre" readonly/>
        <q-input label="Tipo Pago" v-model="factura.tipoPago" readonly/>
      </presentador>
    </q-card-section>
  </q-card>
</template>

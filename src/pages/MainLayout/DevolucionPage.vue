<template>
  <q-page class="column q-ml-md">
    <q-card flat>
      <q-card-section :style="{'padding-bottom': stickyHeight + 'px'}">
        <div class="row items-center">
          ...
          <q-space/>
          <span class="comprobante">ID Devolución: {{ newFactura.id }}</span>
        </div>
        <br>
        <presentador>
        ...
        <div v-intersection="onTotalOculto" class="col-auto row items-end justify-end relative-position">
          ...
          <span
            class="col-auto q-pa-sm item-bordered shadow-2 total"><b>Total devolución: ${{
    ...
    <q-page-sticky v-if="!totalVisible" expand position="bottom">
      <span
        class="shadow-2 total bg-white full-width text-center"><b>Total devolución: ${{
          presentCurrency(total)
        }}</b></span>
    </q-page-sticky>
  </q-page>
